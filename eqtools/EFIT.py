...
class EFITTree(Equilibrium):
    ...
    def __str__(self):
        ...
        try:
            ...
            mes = (
                'EFIT data for shot ' + str(self._shot) + ' from tree ' +
                str(self._tree.upper()) + '\n' + 'timebase ' +
                str(self._time[0]) + '-' + str(self._time[-1]) + 's in ' +
                str(nt) + ' points\n' + str(nr) + 'x' + str(nz) +
                ' spatial grid'
            )
            return mes
    ...
    def getTimeBase(self):
        ...
        if self._time is None:
            try:
                timeNode = self._MDSTree.getNode(
                    self._root+self._afile+':time'
                )
    ...
    def getFluxAxis(self):
        ...
        if self._psiAxis is None:
            try:
                psiAxisNode = self._MDSTree.getNode(
                    self._root + self._afile + ':simagx'
                )
    ...
    def getFluxLCFS(self):
        ...
        if self._psiLCFS is None:
            try:
                psiLCFSNode = self._MDSTree.getNode(
                    self._root + self._afile + ':sibdry'
                )
    ...
    def getElongation(self):
        ...
        if self._kappa is None:
            try:
                kappaNode = self._MDSTree.getNode(
                    self._root+self._afile+':eout'
                )
    ...
    def getUpperTriangularity(self):
        ...
        if self._dupper is None:
            try:
                dupperNode = self._MDSTree.getNode(
                    self._root+self._afile+':doutu'
                )
    ...
    def getLowerTriangularity(self):
        ...
        if self._dlower is None:
            try:
                dlowerNode = self._MDSTree.getNode(
                    self._root+self._afile+':doutl'
                )
    ...
    def getMagR(self, length_unit=1):
        ...
        if self._rmag is None:
            try:
                rmagNode = self._MDSTree.getNode(
                    self._root+self._afile+':rmagx'
                )
    ...
    def getMagZ(self, length_unit=1):
        ...
        if self._zmag is None:
            try:
                zmagNode = self._MDSTree.getNode(
                    self._root+self._afile+':zmagx'
                )
    ...
    def getAreaLCFS(self, length_unit=2):
        ...
        if self._areaLCFS is None:
            try:
                areaLCFSNode = self._MDSTree.getNode(
                    self._root+self._afile+':areao'
                )
    ...
    def getAOut(self, length_unit=1):
        ...
        if self._aLCFS is None:
            try:
                aLCFSNode = self._MDSTree.getNode(
                    self._root+self._afile+':aout'
                )
    ...
    def getRmidOut(self, length_unit=1):
        ...
        if self._RmidLCFS is None:
            try:
                RmidLCFSNode = self._MDSTree.getNode(
                    self._root+self._afile+':rmidout'
                )
    ...
    def getQ0(self):
        ...
        if self._q0 is None:
            try:
                q0Node = self._MDSTree.getNode(
                    self._root+self._afile+':qqmagx'
                )
    ...
    def getQ95(self):
        ...
        if self._q95 is None:
            try:
                q95Node = self._MDSTree.getNode(
                    self._root+self._afile+':qpsib'
                )
    ...
    def getQLCFS(self):
        ...
        if self._qLCFS is None:
            try:
                qLCFSNode = self._MDSTree.getNode(
                    self._root+self._afile+':qout'
                )
    ...
    def getBtVac(self):
        ...
        if self._btaxv is None:
            try:
                btaxvNode = self._MDSTree.getNode(
                    self._root+self._afile+':btaxv'
                )
    ...
    def getBtPla(self):
        ...
        if self._btaxp is None:
            try:
                btaxpNode = self._MDSTree.getNode(
                    self._root+self._afile+':btaxp'
                )
    ...
    def getBpAvg(self):
        ...
        if self._bpolav is None:
            try:
                bpolavNode = self._MDSTree.getNode(
                    self._root+self._afile+':bpolav'
                )
    ...
    def getIpMeas(self):
        ...
        if self._IpMeas is None:
            try:
                IpMeasNode = self._MDSTree.getNode(
                    self._root+self._afile+':pasmat'
                )
    ...
    def getJp(self):
        ...
        if self._Jp is None:
            try:
                JpNode = self._MDSTree.getNode(
                    self._root+self._gfile+':pcurrt'
                )
    ...
    def getBetaT(self):
        ...
        if self._betat is None:
            try:
                betatNode = self._MDSTree.getNode(
                    self._root+self._afile+':betat'
                )
    ...
    def getBetaP(self):
        ...
        if self._betap is None:
            try:
                betapNode = self._MDSTree.getNode(
                    self._root+self._afile+':betap'
                )
    ...
    def getDiamagFlux(self):
        ...
        if self._diamag is None:
            try:
                diamagNode = self._MDSTree.getNode(
                    self._root+self._afile+':diamag'
                )
    ...
    def getDiamagBetaT(self):
        ...
        if self._betatd is None:
            try:
                betatdNode = self._MDSTree.getNode(
                    self._root+self._afile+':betatd'
                )
    ...
    def getDiamagBetaP(self):
        ...
        if self._betapd is None:
            try:
                betapdNode = self._MDSTree.getNode(
                    self._root+self._afile+':betapd'
                )
    ...
    def getDiamagTauE(self):
        ...
        if self._tauDiamag is None:
            try:
                tauDiamagNode = self._MDSTree.getNode(
                    self._root+self._afile+':taudia'
                )
    ...
    def getDiamagWp(self):
        ...
        if self._WDiamag is None:
            try:
                WDiamagNode = self._MDSTree.getNode(
                    self._root+self._afile+':wplasmd'
                )
    ...
    def getDiamag(self):
        ...
        try:
            ...
            data = namedtuple(
                'Diamag',
                ['diaFlux', 'diaBetat', 'diaBetap', 'diaTauE', 'diaWp']
            )
            return data(
                diaFlux=dFlux, diaBetat=betatd, diaBetap=betapd, diaTauE=dTau,
                diaWp=dWp
            )
        except ValueError:
            raise ValueError('data retrieval failed.')
    ...
    def getWMHD(self):
        ...
        if self._WMHD is None:
            try:
                WMHDNode = self._MDSTree.getNode(
                    self._root+self._afile+':wplasm'
                )
    ...
    def getTauMHD(self):
        ...
        if self._tauMHD is None:
            try:
                tauMHDNode = self._MDSTree.getNode(
                    self._root+self._afile+':taumhd'
                )
    ...
    def getPinj(self):
        ...
        if self._Pinj is None:
            try:
                PinjNode = self._MDSTree.getNode(
                    self._root+self._afile+':pbinj'
                )
    ...
    def getWbdot(self):
        ...
        if self._Wbdot is None:
            try:
                WbdotNode = self._MDSTree.getNode(
                    self._root+self._afile+':wbdot'
                )
    ...
    def getWpdot(self):
        ...
        if self._Wpdot is None:
            try:
                WpdotNode = self._MDSTree.getNode(
                    self._root+self._afile+':wpdot'
                )
    ...
    def getBCentr(self):
        ...
        if self._BCentr is None:
            try:
                BCentrNode = self._MDSTree.getNode(
                    self._root+self._gfile+':bcentr'
                )
    ...
    def getEnergy(self):
        ...
        try:
            ...
            Wpdot = self.getWpdot()
            data = namedtuple(
                'Energy', ['WMHD', 'tauMHD', 'Pinj', 'Wbdot', 'Wpdot']
            )
            return data(
                WMHD=WMHD, tauMHD=tauMHD, Pinj=Pinj, Wbdot=Wbdot, Wpdot=Wpdot
            )
        except ValueError:
            raise ValueError('data retrieval failed.')
        ...
        if self._Rlimiter is None or self._Zlimiter is None:
            try:
                ...
                xlim = self._MDSTree.getNode(
                    self._root+self._gfile+':xlim'
                ).data()
                ylim = self._MDSTree.getNode(
                    self._root+self._gfile+':ylim'
                ).data()
                npts = len(xlim)
